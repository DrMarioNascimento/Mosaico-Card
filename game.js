...
const CAMPOS = [
  {
    ...
    opcoes: [
      { id: "anfitriao", txt: "O anfitriao" },
      { id: "convidado", txt: "Um convidado" },
      { id: "nao-se-sabe", txt: "Os fatos nao identificam quem" }
    ]
  },
  {
    id: "vazio",
    rotulo: "O envelope vazio prova o que?",
    resposta: "ausencia",
    opcoes: [
      { id: "furto", txt: "Que o conteudo foi roubado" },
      { id: "ausencia", txt: "Que nao havia conteudo ali" },
      { id: "recusa", txt: "Que alguem recusou a carta" }
    ]
  },
  {
    id: "tinta",
    rotulo: "A tinta recente prova o que?",
    resposta: "recente",
    opcoes: [
      { id: "recente", txt: "Que a assinatura e recente" },
...
const state = {
  ...
  travados: {},
  vez: 0,
  voltasFeitas: { voce: 0, nara: 0, ivo: 0 },
  verbo: null,
  log: ""
};
...
function acabou() {
  return ORDEM.every((id) => state.voltasFeitas[id] >= VOLTAS);
}
...
function arriscar(campoId, valor) {
  ...
  if (ok) {
    state.travados[campoId] = { valor: valor, ok: true };
    say("Arriscou certo. A moeda nao saiu.");
  } else if (state.moedas < PRECO.arriscar) {
    return say("Sem 3 moedas para bancar o erro.");
  } else {
    state.moedas -= PRECO.arriscar;
    state.travados[campoId] = { valor: valor, ok: false };
    say("Arriscou errado · -3. O campo fecha assim para voce.");
  }
  passarVez();
}
