...
assert.equal(bank.elegiveis(8).filter(id => bank.byId[id].deck.maxPlayers === 8).length, 18, "pautas de dezessete ou dezoito cartas usam capacidade oito");
assert.equal(bank.elegiveis(9).filter(id => bank.byId[id].deck.maxPlayers === 9).length, 10, "pautas de dezenove ou vinte cartas usam capacidade nove");
