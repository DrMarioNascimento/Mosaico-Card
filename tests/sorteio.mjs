...
assert.equal(bank.elegiveis(7).filter(id => bank.byId[id].deck.maxPlayers === 7).length, 9, "pautas de quinze ou dezesseis cartas usam capacidade sete");
...
assert.equal(bank.elegiveis(9).filter(id => bank.byId[id].deck.maxPlayers === 9).length, 6, "pautas de dezenove ou vinte cartas usam capacidade nove");
