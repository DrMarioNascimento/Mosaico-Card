...
assert.equal(limitadasASeis.length, 40, "pautas com 13 ou 14 cartas usam capacidade seis");
...
assert.equal(bank.elegiveis(5).filter(id => bank.byId[id].deck.maxPlayers === 5).length, 6, "pautas de onze ou doze cartas usam capacidade cinco");
...
assert.equal(bank.elegiveis(8).filter(id => bank.byId[id].deck.maxPlayers === 8).length, 34, "pautas de dezessete ou dezoito cartas usam capacidade oito");
