...
assert.equal(bank.elegiveis(7).filter(id => bank.byId[id].deck.maxPlayers === 7).length, 6, "pautas de quinze ou dezesseis cartas usam capacidade sete");
assert.equal(bank.elegiveis(8).filter(id => bank.byId[id].deck.maxPlayers === 8).length, 12, "pautas de dezessete ou dezoito cartas usam capacidade oito");
assert.equal(bank.elegiveis(9).filter(id => bank.byId[id].deck.maxPlayers === 9).length, 5, "pautas de dezenove ou vinte cartas usam capacidade nove");
assert.equal(bank.elegiveis(10).filter(id => bank.byId[id].deck.maxPlayers === 10).length, 5, "pautas de 21 ou 22 cartas usam capacidade dez");
assert.equal(bank.elegiveis(11).filter(id => bank.byId[id].deck.maxPlayers === 11).length, 3, "pautas de 23 ou 24 cartas usam capacidade onze");
