...
assert.equal(limitadasASeis.length, 44, "pautas com 13 ou 14 cartas usam capacidade seis");
...
assert.equal(bank.elegiveis(7).filter(id => bank.byId[id].deck.maxPlayers === 7).length, 27, "pautas de quinze ou dezesseis cartas usam capacidade sete");
assert.equal(bank.elegiveis(8).filter(id => bank.byId[id].deck.maxPlayers === 8).length, 42, "pautas de dezessete ou dezoito cartas usam capacidade oito");
