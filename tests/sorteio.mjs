...
assert.equal(bank.elegiveis(5).filter(id => bank.byId[id].deck.maxPlayers === 5).length, 4, "pautas de onze ou doze cartas usam capacidade cinco");
assert.equal(bank.elegiveis(7).filter(id => bank.byId[id].deck.maxPlayers === 7).length, 18, "pautas de quinze ou dezesseis cartas usam capacidade sete");
assert.equal(bank.elegiveis(8).filter(id => bank.byId[id].deck.maxPlayers === 8).length, 26, "pautas de dezessete ou dezoito cartas usam capacidade oito");
assert.equal(bank.elegiveis(9).filter(id => bank.byId[id].deck.maxPlayers === 9).length, 23, "pautas de dezenove ou vinte cartas usam capacidade nove");
assert.equal(bank.elegiveis(10).filter(id => bank.byId[id].deck.maxPlayers === 10).length, 24, "pautas de 21 ou 22 cartas usam capacidade dez");
...
assert.equal(limitadasAQuatro.length, 18, "pautas de nove ou dez cartas usam capacidade quatro");
const limitadasATres = bank.elegiveis(3).filter(id => bank.byId[id].deck.maxPlayers === 3);
assert.equal(limitadasATres.length, 15, "pautas de sete cartas usam capacidade três");
