...
assert.equal(limitadasASeis.length, 42, "pautas com 13 ou 14 cartas usam capacidade seis");
...
assert.equal(bank.elegiveis(8).filter(id => bank.byId[id].deck.maxPlayers === 8).length, 35, "pautas de dezessete ou dezoito cartas usam capacidade oito");
assert.equal(bank.elegiveis(9).filter(id => bank.byId[id].deck.maxPlayers === 9).length, 25, "pautas de dezenove ou vinte cartas usam capacidade nove");
...
assert.equal(limitadasAQuatro.length, 19, "pautas de nove ou dez cartas usam capacidade quatro");
