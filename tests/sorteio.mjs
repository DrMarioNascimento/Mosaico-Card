...
assert.equal(limitadasASeis.length, 30, "pautas com 13 ou 14 cartas usam capacidade seis");
...
assert.equal(bank.elegiveis(7).filter(id => bank.byId[id].deck.maxPlayers === 7).length, 3, "pautas de quinze ou dezesseis cartas usam capacidade sete");
...
assert.equal(bank.elegiveis(10).filter(id => bank.byId[id].deck.maxPlayers === 10).length, 1, "pauta de 21 cartas usa capacidade dez");
assert.equal(bank.elegiveis(11).filter(id => bank.byId[id].deck.maxPlayers === 11).length, 2, "pautas de 23 ou 24 cartas usam capacidade onze");
