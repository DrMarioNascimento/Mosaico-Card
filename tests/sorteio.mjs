...
assert.equal(bank.elegiveis(7).filter(id => bank.byId[id].deck.maxPlayers === 7).length, 16, "pautas de quinze ou dezesseis cartas usam capacidade sete");
assert.equal(bank.elegiveis(8).filter(id => bank.byId[id].deck.maxPlayers === 8).length, 23, "pautas de dezessete ou dezoito cartas usam capacidade oito");
assert.equal(bank.elegiveis(9).filter(id => bank.byId[id].deck.maxPlayers === 9).length, 17, "pautas de dezenove ou vinte cartas usam capacidade nove");
