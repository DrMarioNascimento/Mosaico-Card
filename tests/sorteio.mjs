...
assert.equal(limitadasASeis.length, 43, "pautas com 13 ou 14 cartas usam capacidade seis");
...
assert.equal(bank.elegiveis(5).filter(id => bank.byId[id].deck.maxPlayers === 5).length, 8, "pautas de onze ou doze cartas usam capacidade cinco");
assert.equal(bank.elegiveis(7).filter(id => bank.byId[id].deck.maxPlayers === 7).length, 21, "pautas de quinze ou dezesseis cartas usam capacidade sete");
assert.equal(bank.elegiveis(8).filter(id => bank.byId[id].deck.maxPlayers === 8).length, 37, "pautas de dezessete ou dezoito cartas usam capacidade oito");
assert.equal(bank.elegiveis(9).filter(id => bank.byId[id].deck.maxPlayers === 9).length, 26, "pautas de dezenove ou vinte cartas usam capacidade nove");
...
assert.equal(limitadasAQuatro.length, 20, "pautas de nove ou dez cartas usam capacidade quatro");
const limitadasATres = bank.elegiveis(3).filter(id => bank.byId[id].deck.maxPlayers === 3);
assert.equal(limitadasATres.length, 17, "pautas de sete cartas usam capacidade três");
