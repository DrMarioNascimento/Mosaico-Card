...
const cases = bank.cases.map(entry => ({
  ...
  deck: { status: entry.__derived.blockers.length ? "blocked" : "ready", cards: entry.cards.map(card => ({ ...card, texto: card.text })) },
  reveal: entry.reveal, review: entry.review,
  status: { structural: entry.review.structural, biblical: entry.review.biblical, editorial: entry.review.editorial, playable: entry.__derived.playable },
...
const catalog = { schemaVersion: bank.schemaVersion, catalogVersion: bank.catalogVersion, namespace: bank.namespace, demoIsolated: true, source: { file: path.basename(input), edition: bank.edition, scope: bank.scope }, summary: { cases: cases.length, fields: cases.length * 4, playableCases: cases.filter(entry => entry.status.playable).length }, order: cases.map(entry => entry.id), byId: Object.fromEntries(cases.map(entry => [entry.id, entry])) };
